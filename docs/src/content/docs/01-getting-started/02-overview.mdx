...
Under the hood, Terragrunt passes these values to OpenTofu/Terraform as an auto-loaded `.auto.tfvars.json` file (on Unix) or `TF_VAR_`-prefixed environment variables (on Windows), which OpenTofu/Terraform automatically detects as values for variables defined in `.tf` files.
