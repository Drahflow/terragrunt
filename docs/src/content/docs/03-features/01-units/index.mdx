...
4. Pass any variables defined in the `inputs = { …​ }` block to OpenTofu/Terraform when running it — on Unix via an auto-loaded `.auto.tfvars.json` file, and on Windows via `TF_VAR_`-prefixed environment variables. Notice how the `inputs` block in `stage/app/terragrunt.hcl` deploys fewer and smaller instances than prod.
